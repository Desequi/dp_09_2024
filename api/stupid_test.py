...
def get_is_duplicate(link):
    print('3')
    response = requests.get(link)
    file_path = "data/" + link.split('/')[-1]
    ...
    serch_files = os.listdir('app/descr')
    if os.path.exists(f"app/descr/{key}.pkl"):
        exist = True

    if not exist:
        dumpfile = open(f'app/descr/{key}.pkl', 'wb')
    print('1')
        ...
        if (len(serch_files)) > 0:
            def find_mp(name_dump):
                ...
                if key == name_dump.split('.')[0]:
                    return []
                with open(f'app/descr/{name_dump}', "rb") as dump:
